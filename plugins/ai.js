...
smd(
  {
    ...
    alias: ["sd"],
    desc: "Type Any Text To Create Image With Al",
    category: "ai",
    filename: __filename,
    use: "<text>",
  },
  async (message, input) => {
    try {
      ...
      if (!text) {
        return await message.send("*_I Need The Text To Create An Image._*");
      }
      ...
      if (!response.data) {
        return await message.reply("*Error While Creating An Image!!*");
      }
...
smd(
  ...
  async (message, input) => {
    try {
      const text = input.trim();
      if (!text) {
        return await message.send("*_Please Type Any Question._*");
      }
      ...
      if (!data || !data.status) {
        return await message.reply("*Error In Al.*");
      }
...
smd({
  ...
  if (!text) return message.reply(`*_Please Provide Me A Text_*\n*_Example ${prefix + cmdName} How Many Peoples In Pakistan?_*`);

  try {
    const res = await (await fetch(`https://api.maher-zubair.tech/ai/chatgptv4?q=${text}`)).json();

    if (!res.status === 200) return message.send("*Error Please Try Again Later!*");

    const { result } = res;
    const astro = "𝗚𝗢𝗟𝗗 𝗚𝗣𝗧4\n "
    const tbl = "```";
    await send(message, `${astro}${tbl}${result}${tbl}`);
  } catch (e) {
    return await message.error(`${e}\n\n command: ${cmdName}`, e, `*_Error Please Try Again Later!!_*`);
  }
});
smd({
  pattern: "gemini",
  ...
  if (!text) return message.reply(`*_Please Provide Me A Question_*\n*_Example ${prefix + cmdName} How To Create Gold Md WhatsApp Bot?_*`);
  ...
  try {
    ...
    const { result } = res;
    const astro = "𝗚𝗢𝗟𝗗 𝗠𝗗 𝗔𝗜"
    const tbl = "```";
    await send(message, `${astro}${tbl}${result}${tbl}`);
  } catch (e) {
    return await message.error(`${e}\n\n command: ${cmdName}`, e, `*_Error!! Please Try Again Later_*`);
  }
});
smd(
  {
    ...
    try {
      if (!_0x23f786) {
        return await _0xe6d6e.reply(
          "Hi *" + _0xe6d6e.senderName + "*, Do You Need To Talk With Me?"
        );
      }
      const _0x55bb61 = {
        method: "POST",
  ...
  async (_0x5cb388, _0x302ad5) => {
    try {
      try {
        ...
        if (!_0x557719) {
          return man.reply("Ex:- UMAR Is A Good Boy");
        }
      ...
      await _0x5cb388.error(
        _0x2ef914 + "\n\ncommand: gpt",
        _0x2ef914,
        "*_Not Responding Please Try Again!!!_*"
      );
    }
  }
);
smd(
  ...
  async (_0x42b3b8, _0x3f3887) => {
    try {
      ...
      if (!_0x1b0897) {
        return _0x42b3b8.reply("Provide Me A Question?);
      }
      ...
      if (_0x4743c3 && _0x4743c3.status && _0x4743c3.result) {
        ...
        await _0x42b3b8.send("*_Not Responding*");
      }
    } catch (_0x70fc81) {
  ...
  async (_0x21be87, _0x17d498) => {
    try {
      ...
      try {
        ...
        console.log("ERROR IN DALLE RESPONCE FROM API GOLD-GPT\n", _0x5cee92);
      }
      if (
        Config.OPENAI_API_KEY == "" ||
        !Config.OPENAI_API_KEY ||
      ...
      return await _0x21be87.bot.sendMessage(_0x21be87.chat, {
        ...
        caption: "*---GOLD-MD DALL-E RESULT---*\n" + Config.caption,
      });
    } catch (_0x25b4b9) {
      await _0x21be87.error(
        _0x25b4b9 + "\n\ncommand: dalle",
        _0x25b4b9,
        "*_Not Responding DALL-E_*"
      );
    }
  }
);
smd(
  ...
  async (_0x9bac01, _0x3700d4) => {
    try {
      ...
      if (!_0x2968fd) {
        return await _0x9bac01.reply("*Provide Me A Text To Create An Image?*");
      }
      let _0x24d5e9 = false;
      try {
      ...
      try {
        ...
        console.log("Error To Create Image n", _0x45726b);
      }
      if (
        Config.OPENAI_API_KEY == "" ||
        !Config.OPENAI_API_KEY ||
