...
smd(
  ...
  async (message, match, { cmdName }) => {
    try {
      if (!match)
        return message.reply(`*_Example : ${prefix + cmdName} UMAR_*`);
    ...
    try {
      ...
      if (!text2 || !text1)
        return await message.reply(
          `*_Example : ${prefix + cmdName} GOLD;MD_*`
        );
      return await textToLogoGenerator(
        message,
...
smd(
  ...
  async (message, match, { cmdName }) => {
    try {
      //if (!text) return message.reply(`*_Example : ${prefix+cmdName} GOLD;MD_*`);
      let text1 = match ? match.split(";")[0] : "";
      let text2 = match ? match.split(";")[1] : "";
      if (!text1)
        return await message.reply(`*_Example : ${prefix + cmdName} UMAR_*`);
      return await textToLogoGenerator(
        message,
