...
const sendEditor = async (
  ...
  try {
    ...
    if (!mm || !photo.includes(mm.mtype2))
      return m.reply(`*_I Need The Image To Edit_*`);
    ...
    if (!anu) return m.reply("*_Failed To Create Url Of Image!!_*");
...
for (let i = 0; i < gfxx.length; i++) {
  smd(
    {
      cmdname: gfxx[i],
      infocmd: `Edited Image With ${gfxx[i]} Effect By GOLD MD`,
      type: "editor",
      use: "< image >",
      filename: __filename,
    },
...
\t *Type: _${prefix + smd}_ To Mention Any Image*
_Bot Will Show Results With Single Cmd!_
`;
