  ...
  async (message, input) => {
    try {
      ...
      await message.error(
        error + "\n\nCommand: wabeta",
        error,
        "*Failed To Found WhatsApp Beta Info ,Sorry!!!.*"
      );
    }
  }
);
  ...
  async (message, input) => {
    try {
      ...
      data
        ...
        .catch((error) => {
          console.log(error);
          return message.reply("*Failed to Found Any iOS News.*");
        });
    } catch (error) {
      await message.error(
        error + "\n\nCommand: iosnews",
        error,
        "*Failed to Found Any iOS News.*"
      );
    }
  }
);
  ...
  async (message, input) => {
    try {
      ...
      news
        ...
        .catch((error) => {
          console.log(error);
          return message.reply("*Failed To Found Any Google News.*");
        });
    } catch (error) {
      await message.error(
        error + "\n\nCommand: googlenews",
        error,
        "*Failed To Found Any Google News.*"
      );
    }
  }
);
smd(
  {
    pattern: "nasa",
    desc: "Failed To Found NASA'S Profile Pic.",
    category: "news",
    filename: __filename,
    use: "nasa",
  },
    ...
    try {
      ...
      if (!data || data.status !== 200) {
        return await message.reply(
          "*FailedTo Found NASA'S Profile Pic.*"
        );
      }
      ...
      await message.error(
        error + "\n\nCommand: nasa",
        error,
        "*Failed To Found NASA'S Profile Pic.*"
      );
    }
  }
);
smd(
  ...
  async (message, input) => {
    try {
      ...
      if (!data || data.status !== 200) {
        return await message.reply("*Failed To Get News!!.*");
      }
      ...
      await message.error(error + "\n\nCommand: technews", error, "*Failed To Get News*");
    }
  }
);